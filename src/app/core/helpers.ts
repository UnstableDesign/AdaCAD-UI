import { Cell, Draft, Drawdown, Loom, LoomSettings, defaults, flipDraft, getDraftAsImage, getDraftName, initDraft, unpackDrawdownFromArray, warps, wefts } from "adacad-drafting-lib";
...
export const maxOfPositiveList = (array) => {
  var max = -1;
...
export const findSmallestElmtBiggerThan = (n, array) => {
  var minElmtBiggerThanN = maxOfPositiveList(array) + 1;
  for (var i = 0; i < array.length; i++) {
...
export const findSmallestGap = (array) => {
  var min = minOfList(array);
  var smallestElmtBiggerThan = findSmallestElmtBiggerThan(min, array);
  var max = maxOfPositiveList(array);
  while (smallestElmtBiggerThan - min < 2 && smallestElmtBiggerThan != (max + 1)) {
    min = min + 1;
    smallestElmtBiggerThan = findSmallestElmtBiggerThan(min, array);
  }
 ...
 * @returns 
 */
export const strictMod = (n: number, m: number) => {
  return ((n % m) + m) % m;
}


export const interpolate = (n: number, range: { max: number, min: number }) => {
